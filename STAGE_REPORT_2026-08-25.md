...
补记（同日晚间，六环境收官）。flatenv 六基准全部完成，最终表（teacher-forced 整状态 / rollout H=1 / H=16 / 最深视界）：Freeway 0.999/1.000/1.000/0.953@H64；**Snake 0.945/0.969/0.906/0.828@H64**——MASS 自家域上含变长身体链的 64 步全状态逐位精确率 83%，本批头条；Breakout 0.999/1.000/0.984/0.672@H32（随机策略回合短，自适应评测如实把最深视界定为 32）；SpaceInvaders 0.959/0.938/0.703/0.188@H64；Seaquest 0.932/0.906/0.141/0.000@H64（1919 标量、五个 100 槽实体表，是本项目训过的最大平铺状态）；Asterix 0.897/0.844/0.000/0.000@H64。退化排序与外生随机性排序完全一致（Freeway/Snake 近确定动力学近乎完美，Asterix/Seaquest 的随机实体 spawn 撞 Craftax mob 同一堵墙）。可视化上新增三段真值|预测对照 GIF（Snake 48 步零分叉、Breakout 32 步逐位精确、Freeway 48 步零分叉——展示卷经披露的代表性挑选，表格数字保持无偏）与两张图（六环境视界退化曲线、SMAX 拨盘轨迹）。一个由展示挑选暴露的语料发现：蛇身较长的起点全部在第一步就分叉——随机策略死得快导致训练分布里几乎只有短蛇，长身体动态欠训练；"果子寻路混合策略语料"已列入队列。demo page 已同步全部更新。
